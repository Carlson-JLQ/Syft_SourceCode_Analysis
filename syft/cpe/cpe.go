...
//该代码定义了用于表示和处理 CPE (Common Platform Enumeration) 数据的类型和函数。
import (
	"fmt"
	"regexp"
	"strings"
...
// CPE 类型：包含两个字段：
// Attributes: 表示 CPE 属性的 Attributes 类型对象。
// Source: 表示 CPE 数据来源的 Source 类型字符串 (例如，从 NVD 词典查找、声明的、生成的)。
type CPE struct {
...
// Attributes 类型：表示 CPE 属性的各个部分，包含以下字段：
// Part: CPE 的部分 (例如，a 表示应用程序)。
// Vendor: CPE 的厂商。
// Product: CPE 的产品名称。
// Version: CPE 的版本。
// 其他还有若干可选字段，例如 Update, Edition, TargetSW 等。
...
// 解析一个 CPE 字符串并生成一个 Attributes 对象。
...
// stripSlashes 函数是 sanitize 函数的逆操作。
// 它可以正确地移除后跟允许标点符号的斜杠。
// 这样做的目的是为了正确地解析包含引用字符的 CPE 字符串
// 作用：删除不必要的反斜杠。
// 原理：
// 遍历字符串中的每个字符。
// 如果遇到反斜杠 ()，且它后面跟着允许的标点符号（根据 allowedCPEPunctuation 变量定义），则省略该反斜杠。
// 将剩余的字符组成新的字符串返回。
...
// 作用：转义 CPE 字符串中的标点符号并替换空格。
// 原理：
// 用下划线替换所有空格。
// 遍历字符串中的每个字符。
// 如果遇到允许的标点符号，则在其前面加上反斜杠 () 进行转义。
// 将转义后的字符组成新的字符串返回。
